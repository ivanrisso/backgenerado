  ...
  <div class="space-y-6">
    <div>
      ...
      <p class="text-gray-600">
        Catálogos de teléfonos y domicilios.
      </p>
    </div>
    ...
    <div v-if="!loading && !error" class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <ul class="space-y-2">
          <li v-for="tel in tiposTel" :key="tel.id" class="flex items-center text-gray-700">
            <span class="w-2 h-2 bg-green-500 rounded-full mr-2" />
            {{ tel.nombre }}
          </li>
          <li v-if="tiposTel.length === 0" class="text-gray-400 italic">
            No hay registros
          </li>
        </ul>
      </div>
      ...
      <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <ul class="space-y-2">
          <li v-for="dom in tiposDom" :key="dom.id" class="flex items-center text-gray-700">
            <span class="w-2 h-2 bg-blue-500 rounded-full mr-2" />
            {{ dom.nombre }}
          </li>
          <li v-if="tiposDom.length === 0" class="text-gray-400 italic">
            No hay registros
          </li>
        </ul>
      </div>
