  ...
  <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden mb-8 transform transition-all duration-300">
    ...
    <div class="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white flex justify-between items-center">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">
          Gestione sus productos y servicios de forma profesional.
        </p>
      </div>
      <div v-if="form.activo" class="flex items-center space-x-2">
        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700 border border-green-200 uppercase tracking-tighter">
          <span class="w-1.5 h-1.5 rounded-full bg-green-500 mr-2 animate-pulse" />
          Activo
        </span>
      </div>
    </div>
      ...
      <div class="flex items-start">
        ...
        <div>
          ...
          <p class="text-sm mt-1 opacity-90">
            Eliminar este artículo desactivará su vinculación histórica en los cupones y reservas.
          </p>
        </div>
      </div>
    </div>
    ...
    <form @submit.prevent="handleSubmit">
        ...
        <div class="p-8 grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div class="lg:col-span-8 space-y-6">
            <div>
              ...
              <input
                v-model="form.nombre" type="text" placeholder="Ej: Servicio de Consultoría" required 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  v-model="form.codigo" type="text" placeholder="REF-001" required 
              ...
              <div>
                ...
                >
                  <option :value="TipoArticuloEnum.SERVICIO">
                    Servicio
                  </option>
                  <option :value="TipoArticuloEnum.PRODUCTO">
                    Producto
                  </option>
                  <option :value="TipoArticuloEnum.CONSUMIBLE">
                    Consumible
                  </option>
                </select>
              </div>
          ...
          <div class="lg:col-span-4 flex flex-col justify-start space-y-6 bg-gray-50/50 p-6 rounded-2xl border border-gray-100">
            ...
            <div class="pt-2">
              ...
              <div class="relative rounded-xl shadow-sm">
                ...
                <input
                  v-model="form.precio_venta" type="number" step="0.01" 
        ...
        <div class="mt-4">
          <div class="px-8 border-b border-gray-100 overflow-x-auto scroller-hide">
            <nav class="-mb-px flex space-x-10" aria-label="Tabs">
              <button
                v-for="tab in [{id:'general', name:'General'}, {id:'ventas', name:'Ventas'}, {id:'compras', name:'Compras'}]" 
                :key="tab.id"
                type="button" 
                class="whitespace-nowrap py-5 px-1 border-b-2 font-bold text-xs uppercase tracking-widest transition-all duration-300" :class="[activeTab === tab.id ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-400 hover:text-gray-600 hover:border-gray-300']"
          ...
          <div class="p-8 min-h-[300px] bg-white">
            ...
            <div v-show="activeTab === 'general'" class="max-w-3xl animate-fadeIn">
              <div class="space-y-6">
                <div>
                  ...
                  <textarea
                    v-model="form.description" rows="5" placeholder="Agregue información relevante sobre este artículo..."
                    class="w-full rounded-xl border-gray-200 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-100 transition-all py-3 px-4 resize-none"
                  />
                </div>
            ...
            <div v-show="activeTab === 'ventas'" class="grid grid-cols-1 md:grid-cols-2 gap-12 animate-fadeIn">
              <div class="space-y-6">
                <h4 class="text-xs font-black text-gray-800 uppercase tracking-widest border-b border-gray-100 pb-3">
                  Impuestos de Venta
                </h4>
                <div>
                  <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">IVA Venta predeterminado</label>
                  <select
                    v-model="form.impuesto_venta_id" 
              ...
              <div class="p-6 bg-gray-50 rounded-2xl border border-gray-100 flex flex-col justify-center">
                <div class="text-center">
                  ...
                  <p class="text-xs text-gray-500 mt-2">
                    Al seleccionar este impuesto, se aplicará automáticamente a cada línea de factura cuando se use este artículo.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div class="space-y-6">
                ...
                <div>
                  ...
                  <div class="relative rounded-xl shadow-sm max-w-[200px]">
                    ...
                    <input
                      v-model="form.precio_costo" type="number" step="0.01" 
                ...
                <div>
                  ...
                  <select
                    v-model="form.impuesto_compra_id" 
                    class="w-full rounded-xl border-gray-200 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-100 py-3 px-4 transition-all"
                  >
                    <option :value="undefined">
      ...
      <div class="px-8 py-6 bg-gray-50 border-t border-gray-100 flex justify-between items-center sm:px-10">
        <button
          type="button" class="px-6 py-2.5 text-sm font-bold text-gray-500 hover:text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-100 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-200 shadow-sm" 
          @click="$emit('cancel')"
        >
          Descartar
        </button>
