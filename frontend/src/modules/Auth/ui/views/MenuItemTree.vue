  ...
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <div>
        ...
        <p class="text-gray-600">
          Configuración de estructura y permisos de menú.
        </p>
      </div>
      <button v-if="!showForm" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 flex items-center gap-2" @click="handleNew(null)">
    ...
    <div v-if="showForm">
      <MenuItemForm 
        ...
        :available-parents="flatMenuItems"
        @submit="handleSubmit" 
        ...
        <li v-for="item in menuTree" :key="item.id" class="px-6 py-4">
          <div class="flex justify-between items-start">
            ...
            <div class="flex space-x-2">
              <button class="text-green-600 hover:text-green-900 text-xs font-medium border border-green-200 px-2 py-1 rounded bg-green-50" @click="openCreateChild(item.id)">
                + Hijo
              </button>
              <button class="text-indigo-600 hover:text-indigo-900 text-xs font-medium border border-indigo-200 px-2 py-1 rounded bg-indigo-50" @click="handleEdit(item)">
                Editar
              </button>
              <button class="text-red-600 hover:text-red-900 text-xs font-medium border border-red-200 px-2 py-1 rounded bg-red-50" @click="handleDelete(item.id)">
                Borrar
              </button>
            </div>
          </div>
          ...
          <ul v-if="item.children && item.children.length > 0" class="mt-3 ml-4 border-l-2 border-gray-100 pl-4 space-y-3">
            <li v-for="child in item.children" :key="child.id">
              <div class="flex justify-between items-start">
                ...
                <div class="flex space-x-2">
                  <button class="text-indigo-600 hover:text-indigo-900 text-xs font-medium" @click="handleEdit(child)">
                    Editar
                  </button>
                  <button class="text-red-600 hover:text-red-900 text-xs font-medium pl-2" @click="handleDelete(child.id)">
                    Borrar
                  </button>
                </div>
              </div>
