  ...
  <div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div class="px-8 py-6 border-b border-gray-100 bg-white flex items-center gap-4" :class="{ 'bg-red-50/50': isDeleteMode }">
        <button class="p-2 -ml-2 text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-lg transition-colors" @click="$emit('cancel')">
        ...
        <div v-if="isDeleteMode" class="mb-8 p-4 bg-amber-50 border border-amber-100 rounded-lg flex items-start gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-amber-700 mt-1">
              Se eliminarán también todos los datos vinculados (domicilios, teléfonos, etc.). Esta acción no se puede deshacer.
            </p>
          </div>
        </div>

        <form id="cliente-form" class="space-y-6" @submit.prevent="handleSubmit">
            ...
            <div class="grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-2">
              <div class="col-span-2">
                ...
                <p class="input-helper">
                  Razón social o nombre de fantasía para identificar al cliente corporativo.
                </p>
              </div>
              ...
              <div>
                ...
                <div class="relative">
                  <select v-model="form.tipo_doc_id" required class="input-field appearance-none">
                    ...
                    <option v-for="td in tiposDoc" :key="td.id" :value="td.id">
                      {{ td.nombre }}
                    </option>
                  </select>
              ...
              <div class="col-span-2 border-t border-gray-100 pt-6 mt-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div class="relative">
                      <select v-model="form.condicion_iva_id" class="input-field appearance-none">
                        ...
                        <option v-for="c in condicionesIva" :key="c.id" :value="c.id">
                          {{ c.nombre }}
                        </option>
                      </select>
                      <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-gray-500">
                  ...
                  <div>
                    ...
                    <div class="relative">
                      <select v-model="form.condicion_iibb_id" class="input-field appearance-none">
                        ...
                        <option v-for="c in condicionesIibb" :key="c.id" :value="c.id">
                          {{ c.nombre }}
                        </option>
                      </select>
                      <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-gray-500">
                  ...
                  <div class="sm:col-span-2">
                    ...
                    <p class="input-helper">
                      Número de inscripción en Ingresos Brutos (si corresponde).
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div class="pt-8 flex justify-end gap-3 border-t border-gray-100 mt-8">
            <button type="button" class="btn btn-secondary flex-1 sm:flex-none" @click="$emit('cancel')">
              Cancelar
            </button>
            <button 
              type="submit" 
      ...
      <div v-show="activeTab === 'domicilios'" class="p-6 bg-gray-50/50 min-h-[400px]">
        ...
        <div v-if="!showDomicilioModal" class="space-y-6">
          <div class="flex justify-between items-center px-1">
            ...
            <button v-if="!isDeleteMode" type="button" class="text-[15px] font-medium text-blue-600 hover:text-blue-700 transition-colors" @click="handleAddDomicilio">
              + Agregar Domicilio
            </button>
          </div>
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden divide-y divide-gray-100">
              ...
              <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                <div class="flex-1">
                  <div class="flex items-baseline gap-2 mb-1">
                    ...
                    <span v-if="dom.localidad_id" class="text-sm text-gray-500">(Loc: {{ dom.localidad_id }})</span>
                  </div>
                  <!-- Phones Preview -->
                  <div class="space-y-1">
                ...
                <div v-if="!isDeleteMode" class="flex items-center gap-3 sm:border-l sm:border-gray-100 sm:pl-4">
                  <button type="button" class="text-sm font-medium text-indigo-600 hover:text-indigo-500" @click="handleEditDomicilio(dom)">
                    Editar
                  </button>
                  <button type="button" class="text-sm font-medium text-red-600 hover:text-red-500" @click="handlePreDeleteDomicilio(dom)">
                    Eliminar
                  </button>
                  <button type="button" class="flex items-center gap-1.5 text-sm font-medium text-green-700 bg-green-50 px-3 py-1.5 rounded-full hover:bg-green-100 border border-green-100 transition-colors" @click="navigateToTelefonos(dom.id)">
            ...
            <div v-if="currentDomicilios.length === 0" class="p-12 text-center">
              ...
              <p class="text-[13px] text-gray-400 mt-1">
                Agrega una dirección para comenzar.
              </p>
            </div>
          </div>
        </div>
        ...
        <div v-else class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            ...
            <button type="button" class="text-gray-400 hover:text-gray-600 transition-colors" @click="showDomicilioModal = false">
          ...
          <div class="p-6">
            <DomicilioForm 
              :model-value="editingDomicilio || {} as any" 
              :is-delete-mode="isDomicilioDeleteMode"
              :server-error="domicilioServerError"
      ...
      <div v-if="showAfipSyncModal" class="fixed inset-0 z-[100] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
          <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true" @click="showAfipSyncModal = false" />
                  ...
                  <div class="mt-4 space-y-4">
                    ...
                    <div v-if="afipComparison?.missing_in_local.length" class="space-y-2">
                      <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider">
                        Impuestos en AFIP no registrados localmente
                      </h4>
                      <div class="border border-gray-200 rounded-lg divide-y divide-gray-100">
                        <div v-for="item in afipComparison.missing_in_local" :key="item.afip_id" class="p-3 flex items-center gap-3">
                          ...
                          <div class="flex-1">
                            ...
                            <p v-if="!item.tipo_impuesto_id" class="text-[11px] text-red-500 font-medium">
                              No mapeado en el sistema local
                            </p>
                            <p v-else class="text-[11px] text-green-600 font-medium italic">
                    ...
                    <div v-if="!afipComparison?.missing_in_local.length && !afipComparison?.matches.length" class="text-center py-6">
                      <p class="text-sm text-gray-500 italic">
                        No se encontraron impuestos registrados en AFIP.
                      </p>
                    </div>
                  </div>
                </div>
