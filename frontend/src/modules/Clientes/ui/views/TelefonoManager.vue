...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-end sm:items-center px-4 sm:px-0">
      <h2 class="text-xs font-semibold uppercase tracking-wider text-gray-500 sm:ml-4">
        Lista de Contacto
      </h2>
      <button v-if="!showTelefonoForm" type="button" class="mt-2 sm:mt-0 text-[15px] font-medium text-blue-600 hover:text-blue-500 transition-colors" @click="showTelefonoForm = true">
        + Agregar Teléfono
      </button>
    </div>
    ...
    <div v-if="showTelefonoForm" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mx-0 sm:mx-0">
            ...
            <div class="col-span-1">
              ...
              <div class="relative">
                <select v-model="telefonoForm.tipo_tel_id" class="block w-full rounded-lg border-gray-300 bg-gray-50 py-2.5 px-3 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-[15px] appearance-none transition-shadow">
                  ...
                  <option v-for="t in tiposTel" :key="t.id" :value="t.id">
                    {{ t.nombre }}
                  </option>
                </select>
      ...
      <div class="px-5 py-3 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
        <button type="button" class="px-4 py-2 text-[15px] font-medium text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 shadow-sm transition-all" @click="resetTelefonoForm">
          Cancelar
        </button>
        <button 
          type="button" 
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden divide-y divide-gray-100">
      <template v-if="telefonos.length > 0">
        <div v-for="tel in telefonos" :key="tel.id" class="group flex items-center justify-between p-4 hover:bg-gray-50 transition-colors">
          ...
          <div class="flex items-center gap-4 ml-4">
            <button type="button" class="text-[15px] font-medium text-blue-600 hover:text-blue-500 transition-colors" @click="handleEditTelefono(tel)">
              Editar
            </button>
            <div class="h-4 w-px bg-gray-200" />
            <button type="button" class="text-[15px] font-medium text-red-600 hover:text-red-500 transition-colors" @click="handleDeleteTelefono(tel.id)">
              Eliminar
            </button>
          </div>
        </div>
      ...
      <div v-else class="p-8 text-center">
        ...
        <button class="mt-2 text-sm text-blue-600 hover:underline" @click="showTelefonoForm = true">
          Agregar el primero
        </button>
      </div>
    </div>
