    ...
    <div class="min-h-screen bg-gray-50 font-sans text-gray-900 pb-10">
        ...
        <div class="bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-10">
            <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                <h1 class="text-xl font-semibold tracking-tight text-gray-900 truncate">Gestionar Teléfonos</h1>
                <button @click="goBack" class="text-blue-600 hover:text-blue-500 font-medium text-[17px] flex items-center transition-colors">
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 space-y-8">
            ...
            <div v-if="currentCliente || currentDomicilio" class="space-y-2">
                <h2 class="text-xs font-semibold uppercase tracking-wider text-gray-500 ml-4 mb-2">Información del Contexto</h2>
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden divide-y divide-gray-100">
                     <div v-if="currentCliente" class="p-4 flex flex-col">
                        ...
                        <div class="text-[15px] text-gray-500">{{ currentCliente.cuit?.value || currentCliente.cuit }}</div>
                     </div>
                     <div v-if="currentDomicilio" class="p-4 flex flex-col">
                        ...
                        <div class="font-medium text-[17px] text-gray-900 mt-0.5">{{ currentDomicilio.calle }} {{ currentDomicilio.numero }}</div>
                     </div>
                </div>
            </div>
            ...
            <TelefonoManager v-else :domicilioId="domicilioId" />
        </div>
    </div>
</template>
