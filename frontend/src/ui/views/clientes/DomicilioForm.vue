...
<template>
    <div class="space-y-6">
        ...
        <div v-if="isDeleteMode" class="p-4 bg-amber-50 border border-amber-100 rounded-lg flex items-start gap-3">
            ...
            <div>
                <h3 class="text-sm font-bold text-amber-800">¿Eliminar este domicilio?</h3>
                <p class="text-xs text-amber-700 mt-1">Se borrarán también los teléfonos asociados.</p>
            </div>
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="flex justify-end gap-3 pt-6 border-t border-gray-100 mt-6">
                <button type="button" @click="$emit('cancel')" class="px-5 py-2.5 text-[15px] font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 shadow-sm transition-all focus:outline-none">
                    Cancelar
                </button>
                <button 
                    type="submit" 
