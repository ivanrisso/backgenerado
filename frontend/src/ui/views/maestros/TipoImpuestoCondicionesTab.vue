...
<template>
  <div class="space-y-4">
    <div class="flex justify-between items-center bg-white p-4 rounded-lg border border-gray-100 shadow-sm">
        <div>
            <h4 class="text-sm font-semibold text-gray-900">Condiciones ante este Impuesto</h4>
            <p class="text-xs text-gray-500">Defina las categorías de contribuyentes para este tributo.</p>
        </div>
        <button v-if="!showForm" @click="handleNew" class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
            + Nueva Condición
        </button>
    </div>
    ...
    <div v-if="showForm">
        <CondicionTributariaForm 
            ...
            :serverError="error"
            :fixedTipoImpuestoId="tipoImpuestoId"
            @submit="handleSubmit" 
    ...
    <div v-else>
        ...
        <div v-if="!loading && filteredCondiciones.length === 0" class="text-center py-12 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
            ...
            <h3 class="mt-2 text-sm font-medium text-gray-900">No hay condiciones</h3>
            <p class="mt-1 text-sm text-gray-500">Comience agregando una nueva condición para este impuesto.</p>
        </div>
        ...
        >
            <template #actions="{ item }">
                 <button @click="handleEdit(item)" class="p-1 text-blue-600 hover:bg-blue-50 rounded transition-colors" title="Editar">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                </button>
                <button @click="handlePreDelete(item)" class="p-1 text-red-600 hover:bg-red-50 rounded transition-colors" title="Borrar">
