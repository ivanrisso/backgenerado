...
<template>
  <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
    ...
    <div v-if="isDeleteMode" class="m-6 p-4 bg-amber-50 text-amber-800 border border-amber-200 rounded-xl">
        <p class="font-medium">¿Está seguro que desea eliminar este registro?</p>
        <p class="text-sm mt-1 opacity-80">Esta acción no se puede deshacer y podría afectar el histórico de facturación.</p>
    </div>
    ...
    <form @submit.prevent="handleSubmit">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input v-model="form.nombre" type="text" placeholder="Ej: IVA 21%" required 
                    ...
                    <div>
                        ...
                        <select v-model="form.metodo_calculo" 
                                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-all duration-200">
                            <option :value="MetodoCalculoImpuestoEnum.PORCENTAJE">Porcentaje</option>
                            <option :value="MetodoCalculoImpuestoEnum.FIJO">Importe Fijo</option>
                            <option :value="MetodoCalculoImpuestoEnum.PORCENTAJE_SOBRE_PRECIO">Porcentaje sobre el precio</option>
                            <option :value="MetodoCalculoImpuestoEnum.GRUPO">Grupo de impuestos</option>
                        </select>
                    </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select v-model="form.tipo_uso" 
                                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-all duration-200">
                            <option :value="TipoUsoImpuestoEnum.VENTAS">Ventas</option>
                            <option :value="TipoUsoImpuestoEnum.COMPRAS">Compras</option>
                            <option :value="TipoUsoImpuestoEnum.RETENCION_PAGO_PROVEEDOR">Retención Pago Proveedor</option>
                            <option :value="TipoUsoImpuestoEnum.RETENCION_PAGO_CLIENTE">Retención Pago Cliente</option>
                            <option :value="TipoUsoImpuestoEnum.OTRO">Otros</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Ámbito del impuesto</label>
                        <select v-model="form.ambito_uso" 
                                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-all duration-200">
                            <option :value="AmbitoUsoImpuestoEnum.BIENES">Bienes</option>
                            <option :value="AmbitoUsoImpuestoEnum.SERVICIOS">Servicios</option>
                            <option :value="AmbitoUsoImpuestoEnum.AMBOS">Ambos</option>
                        </select>
                    </div>
                    <div class="grid grid-cols-5 gap-3">
                        <div class="col-span-3">
                            ...
                            <div class="relative rounded-lg shadow-sm">
                                <input v-model="form.importe" type="number" step="0.0001" 
                        ...
                        <div class="col-span-2">
                             ...
                             <input v-model="form.codigo_afip" type="text" placeholder="Ej: 5" required 
                                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-all duration-200" />
                        </div>
                    </div>
                </div>
            ...
            <div class="px-6 border-b border-gray-200">
                <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                    <button type="button" 
                            ...
                            :class="[activeTab === 'definicion' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300', 'whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-all duration-200']">
                        Definición
                    </button>
                    <button type="button" 
                            @click="activeTab = 'avanzadas'"
                            :class="[activeTab === 'avanzadas' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300', 'whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-all duration-200']">
                        Opciones avanzadas
                    </button>
                </nav>
            </div>
            ...
            <div class="p-6 bg-gray-50/30">
                ...
                <div v-show="activeTab === 'definicion'" class="space-y-8 pb-4">
                    <section>
                        ...
                        <p class="text-xs text-gray-500 mb-2 italic">Define cómo se distribuye el importe base e impuesto en facturaciones.</p>
                    ...
                    <section>
                         ...
                         <p class="text-xs text-gray-500 mb-2 italic">Define cómo se distribuye el importe base e impuesto en notas de crédito.</p>
                ...
                <div v-show="activeTab === 'avanzadas'" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input v-model="form.etiqueta_factura" type="text"
                                   class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-all duration-200" />
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Descripción</label>
                            <textarea v-model="form.descripcion" rows="2"
                                      class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-all duration-200"></textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Jurisdicción (Ámbito)</label>
                             <select v-model="form.ambito" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-all duration-200">
                                <option :value="AmbitoImpuestoEnum.NACIONAL">Nacional (Argentina)</option>
                                <option :value="AmbitoImpuestoEnum.PROVINCIAL">Provincial</option>
                                <option :value="AmbitoImpuestoEnum.MUNICIPAL">Municipal</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="space-y-5">
                        <div class="flex items-start space-x-3 p-1">
                             ...
                             <div class="text-sm">
                                ...
                                <p class="text-gray-500 text-xs">El impuesto ya forma parte del precio de venta/costo.</p>
                             </div>
                        </div>
                        <div class="flex items-start space-x-3 p-1">
                             <div class="flex items-center h-5">
                             ...
                             <div class="text-sm">
                                ...
                                <p class="text-gray-500 text-xs">Cálculo en cascada (impuesto sobre impuesto).</p>
                             </div>
                        </div>
                        <div>
                            ...
                            <select v-model="form.categoria_fiscal" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition-all duration-200">
                                ...
                                <option v-for="(label, key) in CategoriaFiscalImpuestoEnum" :key="key" :value="label">{{ label }}</option>
                            </select>
                        </div>
                    </div>
                </div>
        ...
        <div class="px-6 py-5 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
            <button type="button" @click="$emit('cancel')" 
                    class="px-5 py-2 text-sm font-semibold text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-800 transition-all duration-200 focus:ring-2 focus:ring-offset-2 focus:ring-gray-200">
                Descartar
            </button>
