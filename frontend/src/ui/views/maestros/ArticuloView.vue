...
<template>
    <div class="p-8 space-y-8 bg-gray-50/50 min-h-screen">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
                <h1 class="text-3xl font-black text-gray-900 tracking-tight">Artículos y Servicios</h1>
        ...
        <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-200 flex flex-wrap items-center justify-between gap-4">
            <div class="flex items-center space-x-2">
                <button v-for="filter in [{id:'all', label:'Todos'}, {id:TipoArticuloEnum.SERVICIO, label:'Servicios'}, {id:TipoArticuloEnum.PRODUCTO, label:'Productos'}]"
                        :key="filter.id"
                        @click="typeFilter = filter.id"
                        :class="[typeFilter === filter.id ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-transparent text-gray-500 border-transparent hover:bg-gray-50', 'px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border transition-all duration-300']">
                    {{ filter.label }}
                </button>
            </div>
            ...
            <div class="relative flex-grow max-w-md">
                ...
                <input v-model="searchQuery" type="text" placeholder="Buscar por código o nombre..."
        ...
        <div v-if="showForm" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 lg:p-10 pointer-events-none">
            <div class="absolute inset-0 bg-gray-900/60 backdrop-blur-sm pointer-events-auto" @click="showForm = false"></div>
            <div class="relative w-full max-w-5xl max-h-full overflow-y-auto pointer-events-auto shadow-2xl animate-modalUp">
                <ArticuloForm :modelValue="selectedArticulo" 
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <DataTable :items="filteredArticulos" :columns="columns" :loading="loading" :actions="true">
                ...
                <template #cell-tipo="{ item }">
                    <span :class="[getTipoClass(item.tipo), 'px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border']">
                ...
                <template #cell-activo="{ item }">
                    <div class="flex items-center">
                        <div :class="[item.activo ? 'bg-green-500' : 'bg-gray-300', 'h-2.5 w-2.5 rounded-full mr-2']"></div>
                ...
                <template #actions="{ item }">
                    <div class="flex space-x-2">
                        <button @click="handleEdit(item)" 
                                class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors group" title="Editar">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                            </svg>
                        </button>
                        <button @click="handleDelete(item)" 
                                class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors group" title="Eliminar">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
