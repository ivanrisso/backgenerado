  ...
  <div class="space-y-6">
    ...
    <div v-if="!showForm" class="bg-white p-4 rounded-lg shadow-sm border border-gray-200 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <select :value="selectedPaisId" @change="handlePaisChange(Number(($event.target as HTMLSelectElement).value))" class="input-field">
                <option v-for="p in paises" :key="p.id" :value="p.id">{{ p.nombre }}</option>
            </select>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Provincia</label>
            <select :value="selectedProvinciaId" @change="handleProvinciaChange(Number(($event.target as HTMLSelectElement).value))" class="input-field">
