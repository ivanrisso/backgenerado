            ...
            <div class="bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
                ...
                <div class="mb-8 bg-gray-50 p-6 rounded-lg border border-gray-200">
                    ...
                    <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-12 items-end">
                        ...
                        <div class="sm:col-span-1">
                             ...
                             <input type="text" v-model="filters.letra" class="block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3 focus:ring-blue-500 focus:border-blue-500 text-center uppercase" placeholder="A" maxlength="1">
                        </div>
                        
                        <!-- Number -->
                        <div class="sm:col-span-2">
                             <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Nro</label>
                             <input type="text" v-model="filters.nroComprobante" class="block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3 focus:ring-blue-500 focus:border-blue-500">
                        </div>
                    </div>
                </div>
                ...
                <div v-else-if="filteredComprobantes.length === 0" class="text-center py-12">
                    <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                     ...
                     <h3 class="mt-2 text-sm font-medium text-gray-900">No se encontraron comprobantes</h3>
                     <p class="mt-1 text-sm text-gray-500">Intente ajustar los filtros de búsqueda.</p>
                     <div class="mt-6">
                        <button 
                            @click="filters = { fechaDesde: formatDateForInput(firstDayOfMonth), fechaHasta: formatDateForInput(today), cliente: '', tipoComprobanteId: '', nroComprobante: '', letra: '' }" 
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="c in filteredComprobantes" :key="c.id" class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                <div class="font-medium">{{ c.nombre_cliente }}</div>
                                                <div class="text-xs text-gray-500">{{ c.cuit_cliente ? (typeof c.cuit_cliente === 'object' ? c.cuit_cliente.value : c.cuit_cliente) : 'S/C' }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
