            ...
            <div class="bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
                ...
                <form @submit.prevent="submit" class="space-y-8 divide-y divide-gray-200">
                    ...
                    <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6 pt-4">
                        ...
                        <!-- Cliente Autocomplete -->
                         ...
                         <div class="sm:col-span-2">
                             ...
                             <input type="date" v-model="cabecera.fecha_emision" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3 focus:ring-blue-500 focus:border-blue-500">
                         </div>
                         ...
                         <div v-if="isCreditNote" class="sm:col-span-6 bg-yellow-50 p-4 rounded-md border border-yellow-200 mt-2 grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <h4 class="sm:col-span-3 text-sm font-medium text-yellow-800 mb-2">Comprobante Asociado (Requerido para Nota de Crédito)</h4>
                            
                            <!-- Search Helper -->
                             <div class="sm:col-span-3">
                                <label class="block text-xs font-medium text-gray-700 mb-1">Buscar Comprobante del Cliente</label>
                                <select @change="selectAssociatedInvoice(Number(($event.target as HTMLSelectElement).value))" class="block w-full py-1 px-2 border border-gray-300 bg-white rounded-md shadow-sm sm:text-xs">
                                    <option value="">-- Seleccionar Comprobante a Anular --</option>
                            ...
                            <div>
                                ...
                                <input type="number" v-model.number="associatedVoucher.punto_venta" class="mt-1 block w-full py-1 px-2 border border-gray-300 rounded-md shadow-sm sm:text-xs">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700">Número</label>
                                <input type="number" v-model.number="associatedVoucher.numero" class="mt-1 block w-full py-1 px-2 border border-gray-300 rounded-md shadow-sm sm:text-xs">
                            </div>
                         </div>
                        ...
                        <!-- Config -->
                         ...
                         <div class="sm:col-span-2">
                            ...
                            <select v-model="cabecera.concepto_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                <option v-for="c in conceptos" :key="c.id" :value="c.id">{{ c.descripcion }}</option>
                            </select>
                        </div>
                         ...
                         <div class="sm:col-span-2">
                            ...
                            <select v-model="cabecera.moneda_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                <option v-for="m in monedas" :key="m.id" :value="m.id">{{ m.descripcion }}</option>
                            </select>
                        </div>
                    ...
                    <div class="pt-8">
                                        ...
                                        <table class="min-w-full divide-y divide-gray-200">
                                            <thead class="bg-gray-50">
                                                <tr>
                                                    ...
                                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-24">Cant.</th>
                                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-32">Precio Unit.</th>
                                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-32">IVA</th>
                                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-32">Subtotal</th>
                                            ...
                                            <tbody class="bg-white divide-y divide-gray-200">
                                                <tr v-for="(detalle, index) in detalles" :key="index">
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                        <input type="text" v-model="detalle.descripcion" class="focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md" />
                                                    </td>
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                        <input type="number" step="0.01" v-model.number="detalle.cantidad" class="focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md" />
                                                    </td>
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                        <input type="number" step="0.01" v-model.number="detalle.precio_unitario" class="focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md" />
                                                    </td>
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                         <select v-model="detalle.iva_id" class="block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none sm:text-sm">
                                                            <option v-for="iva in ivasTasas" :key="iva.id" :value="iva.id">{{ iva.descripcion }}</option>
                                                        </select>
                                                    </td>
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                        <button type="button" @click="removeItem(index)" class="text-red-600 hover:text-red-900">Eliminar</button>
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                        ...
                        <div class="mt-4">
                             <button type="button" @click="addItem" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-blue-700 bg-blue-100 hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                + Agregar Item
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="pt-8">
                         <dl class="space-y-2 text-right">
                             <div class="flex justify-end gap-4">
                                 <dt class="text-sm font-medium text-gray-500">Subtotal Neto:</dt>
                                 <dd class="text-sm font-medium text-gray-900">$ {{ totalNeto.toFixed(2) }}</dd>
                             </div>
                             <div class="flex justify-end gap-4">
                                 <dt class="text-sm font-medium text-gray-500">IVA Total:</dt>
                                 <dd class="text-sm font-medium text-gray-900">$ {{ totalIva.toFixed(2) }}</dd>
                             </div>
                             <div class="flex justify-end gap-4 border-t border-gray-200 pt-2">
                                 <dt class="text-base font-bold text-gray-900">Total:</dt>
                                 <dd class="text-base font-bold text-blue-600">$ {{ totalGeneral.toFixed(2) }}</dd>
                             </div>
                         </dl>
                    </div>
                    ...
                    <div class="pt-5">
                        <div class="flex justify-end">
                            <button type="button" @click="router.back()" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none">
                                Cancelar
                            </button>
